...
menu.querySelectorAll('a').forEach(link => link.addEventListener('click', () => {
  menu.classList.remove('open'); menuButton.setAttribute('aria-expanded', 'false'); menuButton.textContent = '☰';
}));
...
function simulate() {
  const solar = +fields.sun.value, ambient = +fields.ambient.value, water = +fields.water.value;
  const gain = (solar / 100) * 5.2 + Math.max(0, ambient - water) * .13;
  const result = Math.min(40, water + Math.max(.15, gain));
  outputs.sun.textContent = `${solar}%`; outputs.ambient.textContent = `${ambient}°C`; outputs.water.textContent = `${water}°C`;
...
Object.values(fields).forEach(input => input.addEventListener('input', simulate)); simulate();

const counters = document.querySelectorAll('[data-count]');
const counterObserver = new IntersectionObserver(entries => entries.forEach(entry => {
  if (!entry.isIntersecting) return;
  const node = entry.target, target = Number(node.dataset.count), start = performance.now();
